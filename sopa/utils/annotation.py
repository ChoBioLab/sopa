...
def tangram_annotate(
    ...
    **kwargs,
    ...
    Args:
        ...
        output_dir: Directory to save training data. If None, training data is not saved.
    """
...
class MultiLevelAnnotation:
    ...
    def run(self, output_dir=None):
        ...
        if save_training_data:
            ...
            maps_dir = output_dir / "tangram_maps"
            maps_dir.mkdir(parents=True, exist_ok=True)
            log.info(f"Will save training data to {maps_dir}")
        ...
        for level in range(self.levels):
            ...
            # Create level directory if saving training data
            level_dir = None
            if save_training_data:
                level_dir = maps_dir / f"level_{level}"
                level_dir.mkdir(exist_ok=True)

            if level == 0:
                # For level 0, run on all cells
                self.run_group(level, output_dir=level_dir)
            else:
